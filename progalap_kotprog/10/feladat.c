#include <stdio.h>
#include <stdlib.h>
char *dekodolo (char kulcsszo[], char kodolando[]) {
	int i,index=0,eltolas=0,kulcs[26];
	char *kodolt=malloc(sizeof(char)*256);
...
char *kodolo (char kulcsszo[], char kodolando[]) {
...
int main () {
	FILE *be, *ki;
	char kulcsszo[27], mod, kodolando[256], *kodolt;
	...
	if (mod=='K') {
		kodolt=kodolo(kulcsszo, kodolando);
	} else {
		kodolt=dekodolo(kulcsszo, kodolando);
	}
	fprintf(ki,"%s\n",kodolt);
	fclose(be);
	fclose(ki);
